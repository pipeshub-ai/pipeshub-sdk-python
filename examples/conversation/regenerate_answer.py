"""Regenerate a conversation answer with a streaming response."""
    ...
    async with SDK(
        ...
        async for event in new_conversation_stream:
            ...
            if event.event == "RUN_FINISHED" and data:
                conversation = data["result"]["conversation"]
        ...
        regenerate_stream = await client.conversations.regenerate_answer_async(
            conversation_id=conv_id,
            message_id=msg_id,
        )
        async for event in regenerate_stream:
            data = event.data
