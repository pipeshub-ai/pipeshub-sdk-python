...
AgentStreamCreateConversationRequestProtocol = Literal["agui",]
...
class AgentStreamCreateConversationRequestTypedDict(TypedDict):
    r"""Request body for `POST /agents/{agentKey}/conversations/stream`.
    ...
    """
    protocol: NotRequired[AgentStreamCreateConversationRequestProtocol]
    r"""AG-UI is the only supported wire protocol. When present must be
    `\"agui\"`. Omitting the field is equivalent — the server always
    uses the AG-UI vocabulary (see `AgentStreamSSEEvent`). Kept in
    the schema for backward compatibility with callers that already
...
class AgentStreamCreateConversationRequest(BaseModel):
    ...
    protocol: Optional[AgentStreamCreateConversationRequestProtocol] = None
    r"""AG-UI is the only supported wire protocol. When present must be
    `\"agui\"`. Omitting the field is equivalent — the server always
    uses the AG-UI vocabulary (see `AgentStreamSSEEvent`). Kept in
    the schema for backward compatibility with callers that already
