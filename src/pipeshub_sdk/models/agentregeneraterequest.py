...
AgentRegenerateRequestChatMode = Literal["quick",]


AgentRegenerateRequestProtocol = Literal["agui",]
...
class AgentRegenerateRequestTypedDict(TypedDict):
    r"""Regeneration payload for a scoped agent conversation. The public
    gateway requires `chatMode: quick`; all other fields retain the
    semantics defined by `RegenerateRequest`.

    """

    chat_mode: AgentRegenerateRequestChatMode
    ...
    """
    protocol: NotRequired[AgentRegenerateRequestProtocol]
    r"""AG-UI is the only supported wire protocol. When present must be
    `\"agui\"`. Omitting the field is equivalent — the server always
    uses the AG-UI vocabulary. Kept in the schema for backward
    compatibility with callers that already send it.
...
class AgentRegenerateRequest(BaseModel):
    ...
    chat_mode: Annotated[
        AgentRegenerateRequestChatMode, pydantic.Field(alias="chatMode")
    ]
    ...
    protocol: Optional[AgentRegenerateRequestProtocol] = None
    r"""AG-UI is the only supported wire protocol. When present must be
    `\"agui\"`. Omitting the field is equivalent — the server always
    uses the AG-UI vocabulary. Kept in the schema for backward
    compatibility with callers that already send it.
...
try:
    AgentRegenerateRequest.model_rebuild()
except NameError:
    pass
